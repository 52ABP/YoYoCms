...
(function (abp, angular) {
    ...
    abp.ng.http = {
        ...
        handleNonAbpErrorResponse: function handleNonAbpErrorResponse(response, defer) {
            if (response.config.abpHandleError !== false) {
                switch (response.status) {
                    case 401:
                        abp.ng.http.handleUnAuthorizedRequest(abp.ng.http.showError(abp.ng.http.defaultError401), abp.appPath);
                        break;
                    case 403:
        ...
        handleResponse: function handleResponse(response, defer) {
    ...
    abpModule.config(['$httpProvider', function ($httpProvider) {
        $httpProvider.interceptors.push(['$q', function ($q) {
            ...
            return {
                ...
                'response': function response(_response) {
                    if (!_response.data || !_response.data.__abp) {
                        //Non ABP related return value
                        return _response;
                    }

                    var defer = $q.defer();
                    abp.ng.http.handleResponse(_response, defer);
                    return defer.promise;
                },

                'responseError': function responseError(ngError) {
                    var defer = $q.defer();
